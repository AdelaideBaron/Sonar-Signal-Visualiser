...
class WaveGridTest {
  ...
  @Test
  void shouldDrawVerticalGridLineEveryTwentyFivePixels() throws Exception {
  ...
  @Test
  void shouldDrawHorizontalGridLineEveryTwentyFivePixels() throws Exception {
  ...
  @Test
  void shouldDrawSineWaveAboveCentreLineAtFirstPeak() throws Exception {
    ...
    int expectedY = sineWave.getY(peakX, PANEL_WIDTH, centreY);

    Color pixelColour = getPixelColour(image, peakX, expectedY);
  ...
  private WaveGrid createWaveGridOnEdt() throws InvocationTargetException, InterruptedException {
    ...
    SwingUtilities.invokeAndWait(() -> result[0] = new WaveGrid(10));
