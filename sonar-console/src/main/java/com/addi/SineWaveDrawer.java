...
public final class SineWaveDrawer {
  ...
  public static void draw(Graphics2D graphics, SineWave sineWave, int width, int height) {
    if (width <= 0 || height <= 0) {
      return;
    }
    ...
    int previousX = 0;
    int previousY = sineWave.getY(previousX, width, centreY);

    for (int x = 1; x < width; x++) {
      int y = sineWave.getY(x, width, centreY);
