 ...
 *
 * <p>Size: 700 × 350 Background: black Grid spacing: 25 Grid colour: RGB(0, 100, 0) Centre line:
 * bright green Wave: bright green
 */
public class WaveGrid extends JPanel {
  ...
  public WaveGrid(double frequency) {
    ...
    animationTimer = new Timer(TIMER_DELAY_MILLISECONDS, event -> advancePhase());
  }

  private void advancePhase() {
    double secondsPerTick = TIMER_DELAY_MILLISECONDS / 1000.0;

    double phaseChange = 360.0 * frequency * secondsPerTick;

    double nextPhase = getPhaseDegrees() + phaseChange;
  ...
  private void validateFrequency(double frequency) {
    if (!Double.isFinite(frequency) || frequency <= 0.0) {
      throw new IllegalArgumentException("Frequency must be a positive, finite number.");
    }
  }
  ...
  protected void paintComponent(Graphics graphics) {
    ...
    try {
      ...
      SineWaveDrawer.draw(graphics2D, sineWave, getWidth(), getHeight());
    } finally {
      graphics2D.dispose();
    }
  }
  ...
  private void drawCentreLine(Graphics2D graphics) {
    ...
    graphics.drawLine(0, centreY, getWidth(), centreY);
  }
}
